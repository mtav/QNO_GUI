...
* **Replenish stack:** If this is enabled, the draw pile gets replenished when it runs out by taking all cards from the discard pile (except the current top visible card) and reshuffling them. This can make the game last a very long time and the game will only end once someone manages to discard all their cards. If it is disabled and the draw pile runs out, the game ends. The winner(s) is(are) then the ones with the least cards left.
